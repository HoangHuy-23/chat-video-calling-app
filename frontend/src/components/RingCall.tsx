...
function RingCall({ showRingCall, setShowRingCall }: RingCallProps) {
  // listen call
  const { ongoingCall, handleJoinCall, handleHangup } = useCallStore();
  ...
  return (
    <div
      className={`${
        showRingCall ? "" : "hidden"
      } absolute flex flex-col z-20 w-60 border bg-primary border-base-300 rounded-md h-72 bottom-16 right-1 shadow-sm shadow-slate-800`}
    >
      ...
      <div className="flex flex-col grow items-center justify-around w-full py-4">
        ...
        <h3 className="text-lg font-semibold">
          {ongoingCall?.participants?.caller?.userId || "NO"}
        </h3>
